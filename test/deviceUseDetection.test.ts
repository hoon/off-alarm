...
beforeAll(async () => {
  ...
  duck = await DuckDb.create(':memory:')

  await initDuckTables(duck)
  await getDevicePowerReadings(influxDb, duck)
})

describe('getDevicePowerStats', () => {
  test('are data rows in duckdb', async () => {
    const rows = await duck.all(`SELECT * FROM device_power`)
    console.log(rows)
    expect(rows.length).toBeGreaterThan(0)
  })
  test('able to get something back', async () => {
    const stats = await getDevicePowerStats(duck, {
      forSec: 300,
      untilTime: Date.now(),
    })
    console.log(stats)
